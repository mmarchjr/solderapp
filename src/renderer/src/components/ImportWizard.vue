      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div v-else>
            ...
            <!-- No files warning -->
            <div
              v-if="drillFiles.length === 0 && outlineFiles.length === 0 && !loading"
...
async function importFiles() {
  ...
  if (hasOutline) {
    progressSteps.value.push({
      label: `Parse outline: ${selectedOutlineName.value}`,
      status: 'pending'
    })
  }
  ...
  try {
    ...
    for (let i = 0; i < selectedDrills.length; i++) {
      ...
      console.log(
        `[ImportWizard] Parsing drill file ${i + 1}/${selectedDrills.length}:`,
        drillFile.name
      )
      ...
      console.log(
        `[ImportWizard] Parsed ${parsed.drills.length} drill points from ${drillFile.name}`,
        parsed.toolSizes
      )
    ...
    if (allDrills.length > 0) {
      ...
      console.log(
        `[ImportWizard] Setting ${allDrills.length} total drill points, filename: ${filename}`
      )
    ...
    if (hasOutline) {
      ...
      if (outlineData) {
        ...
        console.log(
          `[ImportWizard] Outline raw (${outlineText.length} chars):`,
          outlineText.substring(0, 500)
        )
    ...
    progressSubtitle.value =
      `${allDrills.length} drill points loaded` + (hasOutline ? `, outline loaded` : '')
...
function parseGerberTextInline(text) {
  ...
  console.log(
    `[GerberParser] Format: ${integerDigits}:${decimalDigits}, unitMode=${unitMode}, leadingZeros=${useLeadingZeros}`
  )
  ...
  for (const line of lines) {
    ...
    if (hasX || hasY || operation !== null) {
      ...
      console.log(
        `[GerberParser] line="${line}" rawX=${rawX} rawY=${rawY} parsedX=${newX.toFixed(3)} parsedY=${newY.toFixed(3)} op=${operation} mode=${interpolateMode}`
      )

      if (operation === 'draw' && interpolateMode === 1) {
        const x1 = toMm(currentX),
          y1 = toMm(currentY)
        const x2 = toMm(newX),
          y2 = toMm(newY)
        if (x1 !== x2 || y1 !== y2) {
        ...
        const cx = toMm(currentX + iVal),
          cy = toMm(currentY + jVal)
        const startX = toMm(currentX),
          startY = toMm(currentY)
        const endX = toMm(newX),
          endY = toMm(newY)
